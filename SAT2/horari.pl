...
% Auxiliary function very necessary!!!!!
...
defineCourseHR:- course(C), room(R), day(D), hour(H), expressAnd(courseHourR-C-D-H-R, [courseRoom-C-R, courseHour-C-D-H]), fail.
defineCourseHR.

threePerWeek:- course(C), findAll(courseHour-C-D-H, (day(D), hour(H)), Lits), exactly(3, Lits), fail.
threePerWeek.

differentDays:- course(C), day(D), findAll(courseHour-C-D-H, hour(H), Lits), atMost(1, Lits), fail.
differentDays.

noOverlapping:- student(S), day(D), hour(H), findAll(courseHour-C-D-H, (student(S, L), member(C,L)), Lits), atMost(1,Lits), fail.
noOverlapping.

sameRoom:- course(C), findAll(courseRoom-C-R, (courseRooms(C, L), member(R, L)), Lits), atLeast(1, Lits), fail.
sameRoom:- course(C), findAll(courseRoom-C-R, room(R), Lits), atMost(1, Lits), fail.
sameRoom.

noOverlappingR:- room(R), day(D), hour(H), findAll(courseHourR-C-D-H-R, course(C), Lits), atMost(1, Lits), fail.
noOverlappingR.

threeHoursDay:- student(S), day(D), student(S,L), findAll(courseHour-C-D-H, (member(C,L), hour(H)), Lits), atMost(3, Lits), fail.
threeHoursDay.

atMostKLate(K):- findall(late-D, day(D), Lits), atMost(K, Lits).
...
searchLoop(K) :-
  ...
  shell('picosat -l 100000 -v -o model infile.cnf', Result),  % if sat: Result=10; if unsat: Result=20. Set also picosat limit
  treatResult(Result, K),!.
  
treatResult( 0, K):- nl, write('Time limit! Last satisfying K='), K1 is K+1, write(K1), nl, see(model2), symbolicModel(M), seen, displaySol(M),nl,halt.
treatResult( 0, 5):- write('Unsatisfiable!'), nl, halt.
treatResult(20, K):- nl, write('Unsatisfiable for K='), write(K), write(' Last satisfying K='), K1 is K+1, write(K1), nl, see(model2), symbolicModel(M), seen, displaySol(M),nl,halt.
